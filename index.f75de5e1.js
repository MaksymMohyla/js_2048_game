...
якщо гра програна, керування в обробнику подій блокується */ let isGameOver = false;
class Game {
    ...
    якщо рух в даному напрямку неможливий */ const previousState = JSON.parse(JSON.stringify(this.state));
        for(let row = 0; row < 4; row++){
            let col = 0;
            // ітеруємо по всіх рядках зліва направо
            for(let i = 1; i < 4; i++){
                // якщо зустрічаєм непусту клітинку
                if (this.state[row][i] !== 0) for(let j = i; j > col; j--){
    ...
    moveUp() {
        ...
        for(let col = 0; col < 4; col++){
            ...
            for(let i = 1; i < 4; i++){
                if (this.state[i][col] !== 0) for(let j = i; j > row; j--){
    ...
    isAnyMovePossible() {
     ...
     сусідні плитки з однаковими числами, то є можливий хід */ for(let row = 0; row < 4; row++)for(let col = 0; col < 4; col++){
    ...
    тому викликаєм його двічі для кнопки старт */ for(let i = 0; i < 2; i++)this.addRandomTile();
    }
    addRandomTile() {
        let row, col;
        do {
    ...
    ітеруєм по всіх рядках і колонках поля */ for(let row = 0; row < 4; row++)for(let col = 0; col < 4; col++){
            const cell = gameTable.rows[row].cells[col];
        ...
        таке ж число як в масиві, тому ними зручно маніпулювати) */ cell.className = "field-cell";
            if (this.state[row][col]) cell.classList.add(`field-cell--${this.state[row][col]}`);
        }
    }
    updateScore() {
